...
func createFileHandler(o wmenu.Opt) error {
	...
	if file, err := os.Create(fmt.Sprintf("%s/%s.json", filesDir, text)); err != nil {
		log.Fatal(err)
	} else {
		file.Close()
	}
...
func writeToFileHandler(o wmenu.Opt) error {
	testObject := fileStruct{
		Username:    "shkunov",
		Password:    "test",
		DisplayName: "Андрей Шкунов",
		Email:       "shkunov.a.v@edu.mirea.ru",
		Role:        "student",
		Group:       "BSBO-01-19",
		Age:         21,
	}
	...
	jsonString, err := json.Marshal(testObject)
